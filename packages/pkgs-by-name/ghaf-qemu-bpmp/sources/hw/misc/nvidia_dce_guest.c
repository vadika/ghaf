...
/*
 * Reverse (async) window -- the DCE R5's unsolicited ch_type=3 notifications
 * (vblank, and the flip-completion event a guest modeset blocks on). A helper
 * thread poll()s /dev/dce-host, read()s one event, publishes it here and bumps
 * EVT_SEQ. The guest proxy watches EVT_SEQ, consumes the event, and writes the
 * seq back to EVT_ACK. The thread holds off publishing the next event until
 * ACK catches up, so no event is overwritten unseen.
 */
#define FWD_SIZE  0x3000  /* forward window the guest's sync bulk copy touches;
			   * the reverse window sits above it so a sync send
			   * never clobbers EVT_SEQ/EVT_ACK. */
...
/* Mirror of struct dce_host_event (dce-host-proxy.h): one read() per event. */
...
/*
 * Layout of the single write() fop payload consumed by /dev/dce-host
 * (dce-host-proxy.h, Task 1a). Must stay in lockstep with that struct:
 * iface then tx{data,size} then rx{data,size} then ret, no packing pragmas.
 */
struct dce_host_msg
{
	uint32_t iface;
...
/*
 * Reverse-doorbell pump. Blocks in poll() on /dev/dce-host until the host proxy
 * has an async DCE notification queued, reads exactly one, and publishes it into
 * the reverse window for the guest proxy to pick up. Gated on EVT_ACK so a slow
 * guest never loses an event -- unconsumed events stay buffered in the host
 * proxy's ring instead.
 *
 * ponytail: poll(2) + a 500us ack-wait spin, no guest IRQ. nvidia-drm's flip
 * wait is 3s, so this latency is irrelevant; wire a GIC SPI only if a future
 * hot path needs it.
 */
static void *nvidia_dce_guest_evt_thread(void *opaque)
{
	...
	while (!qatomic_read(&s->stopping)) {
		...
		if (r < (ssize_t)(2 * sizeof(uint32_t))) {
			 ...
			 * debugging. The kernel side now retains the event on
			 * a failed copy, so a retry will see it again. */
...
type_init(nvidia_dce_guest_register_types)
	...
	/*
	 * Create the Nvidia DCE guest device.
	 *
	 * Forward (sync) path via the doorbell, plus the reverse (RM_NOTIFY)
	 * window drained by the poll thread started in instance_init. Delivery to
	 * the guest is poll-based (guest proxy watches EVT_SEQ), so no sysbus IRQ
	 * is wired here.
	 */
