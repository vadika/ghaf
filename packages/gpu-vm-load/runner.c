...
//
// gpu-vm-load: a self-contained CUDA Driver API host runner for the gpu-vm
// passthrough smoke test. It embeds the burn kernel PTX (linked in as a binary
// blob), launches it in a loop for a number of seconds, and prints the GPU's
// compute capability. Deliberately uses the Driver API with hand-declared
// prototypes so it needs no CUDA toolkit headers and links only against the
// native L4T libcuda.so.1 (RPATH-wired at build), so it runs on the guest with
// no LD_LIBRARY_PATH or nvcc toolchain present.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
