...
# All platforms must use the evaluatedConfig pattern with a profile's gpuvmBase.
#
# NOTE: gpuvmBase (analogous to netvmBase) is exported by the orin profile,
# which also wires ghaf.virtualization.microvm.gpuvm.evaluatedConfig. This VM
# stays inert unless that wiring is present: enable is only ever true when
# ghaf.hardware.nvidia.passthroughs.gpu_vm.enable is true (Orin AGX only), and
