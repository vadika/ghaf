...
# Disp VM Base Module
#
# Minimal base configuration for the display-passthrough VM (Orin AGX,
# experiment/orin-two-vm-host1x concurrent build). Mirrors gpuvm-base.nix but
# drops everything GPU-compute-specific (CUDA userspace, gpu-vm-load smoke
# test, /dev/nvgpu node-access service): disp-vm owns no GPU, only the
# display/scanout passthrough wired via ghaf.hardware.definition.dispvm.extraModules.
#
# Usage in profiles:
#   lib.nixosSystem {
...
in
{
  ...
  ghaf = {
    ...
    # ponytail: no dedicated dispvm givc role exists; enable transport only,
    # same minimal choice as gpuvm-base.
    givc = {
  ...
  microvm = {
    ...
    # ponytail: vcpu pinned to 4 to match tegra234-dispvm.dts's cpus node
    # (same dummy-virt boilerplate as tegra234-gpuvm.dts); mem is a display-
    # only default, smaller than the GPU VM's 6000.
    vcpu = 4;
